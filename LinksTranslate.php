...
class LinksTranslate {
	...
	public function getInterwiki ($article, $namespace = 0) {
		...
		if ($row = $result->fetch_array()) {
			if ($row['page_is_redirect'] == 0) {
				...
				$redirect = MediaWikiPage::resolveRedirectFromPageId($this->sourceProject,  $row['page_id']);
	...
	public function translateLinks () {
		...
		$page = new MediaWikiPage($this->sourceProject, $this->pageTitle, $this->namespace);
		...
		while ($row = $result->fetch_array()) {
			...
			$page = new MediaWikiPage($this->sourceProject, $title, $ns);
			$links[] = array(
